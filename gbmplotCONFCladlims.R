          ...
          plot.layout = c(3, 4), ...) 
{
  ...
  for (j in c(1:n.plots)) {
    ...
    if (common.scale) {
      ...
      if (is.factor(data[,gbm.call$gbm.x[k]])) {
        plot(CIplot[[1]],CIplot$y - mean(CIplot$y),ylim=c(-.5,1), type='l', xlab = x.label, ylab = y.label, ...) 
        plot(CIplot[[1]], CIplot$CIhi - mean(CIplot$y),ylim=c(-.5,1), type='l', lwd = .5, add=TRUE, border="gray49",axes = FALSE, xlab = "", ylab = "")
        plot(CIplot[[1]], CIplot$CIlo - mean(CIplot$y),ylim=c(-.5,1), type='l', lwd = .5, add=TRUE, border="gray49",axes = FALSE, xlab = "", ylab = "")
        legend("topleft",paste(round(gbm.object$contributions[j, 2], 1),"%", sep = ""), bty="n", adj=.5) 
        
      }else {
        if (pred.names[[k]] == "NearStructures") {
          ...
          plot(CIplot[[1]], CIplot$y - mean(CIplot$y), type="l", ylim=c(-.5,1), xlab = x.label, ylab = y.label, ...)
          lines(CIplot[[1]], CIplot$CIhi - mean(CIplot$y),col="gray49",lty=3, ylim=c(-.5,1))
          lines(CIplot[[1]], CIplot$CIlo - mean(CIplot$y),col="gray49", lty=3, ylim=c(-.5,1))
          legend("topleft", paste(round(gbm.object$contributions[j, 2], 1), "%", sep = ""), bty="n", adj=.5) 
        }
      }
    }
